...
const limitmaxrecord = 5 * 60; // 5 min
...
function parseInputFormat(input: string): Trip {
  ...
  if (trips === null) {
    throw new Error('Invalid input format - need two lines of data');
  }
  ...
  readline.question(question, (input: string) => {
    ...
    try {
      ...
      interval = setInterval(() => {
        ...
        if (i > 0 && times[i] - times[i - 1] > limitmaxrecord) {
          throw new Error('The interval is too long');
        }
